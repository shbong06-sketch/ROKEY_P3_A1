...
### 2-1. 9월 24일 추가: 운반 중 팔레트 감시 예외 처리 (한 군데)

| 위치 | 추가한 것 | 하는 일 |
|---|---|---|
| `run()` 메인 루프의 `runtime.motion.hold()` | `try/except RuntimeError` 5줄 + `SimulationRuntime` 필드 `hold_fault_logged` | 운반 중 팔레트가 30 mm 넘게 밀리면 팀 `robot_motion.check` 가 예외를 던지는데, 지금까지는 그 예외가 main loop 밖으로 나가 **Isaac 전체가 종료**되었다(24차 실측, `HOLD: 운반 중 팔레트가 30.1 mm 미끄러졌습니다`). 이제는 한 번만 오류 로그를 남기고 계속 돈다 |

- 팔은 그대로 유지된다. `hold()` 는 예외를 던지기 전에 관절 목표를 다시 보내지 못하지만, 관절 목표는 물리 드라이브에 이미 남아 있어 팔이 처지지 않는다.
- 안전 판단은 그대로다. 다음 PLACE 명령이 오면 같은 검사가 `update_operation` 안에서 다시 예외를 던지고, 그건 원래대로 `fail_operation(MOTION_FAILED)` 로 **결과 메시지로** 보고된다. 즉 "실패를 숨기는" 것이 아니라 "실패를 앱 종료 대신 결과로 보고" 하게 한 것이다.
- 팀 판단이 필요한 부분: 30 mm 기준 자체(`PALLET_SLIP_TOL`)와, 운반 중 감시를 계속할지는 팀 코드(`robot_motion.py`)라 손대지 않았다.
