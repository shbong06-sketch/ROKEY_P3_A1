...
| carter1 주행 | 외부 ROS 2 노드 | `path_runner_smooth` (`path_smooth.launch.py`) | 통로 탈출 → 곡선 → 컨베이어(검수) 앞 정지 |
| carter2 주행 | 외부 ROS 2 노드 | `path_runner` (`carter2_dock.launch.py`) | 맵 밖에서 통로 진입 → 도킹 창 안 저속 정지 |
