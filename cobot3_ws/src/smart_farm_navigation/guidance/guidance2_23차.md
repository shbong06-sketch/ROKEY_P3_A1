# guidance2_23차 — 주행·도킹·Place 전 구간 (고피에서 지시, 내피에서 Nav2)

- 작성일: 2026-09-23, 브랜치 `feature/Inspection-Place-nav2`. 도킹 중 컨베이어에 걸린 원인을 고친 판이며 22차는 `guidance/past/` 로 옮김.
...
주행은 되었으나 도킹 중 카터가 컨베이어 구조물에 걸려 멈췄다. bag 을 되짚어 본 결과는 다음과 같다.

| 시점 | 카터 map 자세 | 상태 |
|---|---|---|
| 도킹 시작 | (−2.19, −1.51) yaw +63.5도 | 접근 지점 |
| 후진 시작 | (−2.26, −1.80) yaw +85.9도 | 거의 직각, 정상 |
| 후진 끝 | (−2.33, −2.86) yaw +60.6도 | **1 m 후진하는 동안 방향이 25도 틀어짐** |
| 15초 뒤 | (−2.32, −2.87) yaw +73.1도 | 제자리 회전으로 되돌리는 중 |
| 실패 | (−2.32, −2.87) yaw +73.4도 | **30초 동안 명령을 내는데도 전혀 움직이지 않음** |

원인은 두 가지다.

1. **후진 중 방향이 틀어짐.** 목표점만 겨누는 조향이라 목표점이 가까워질수록 조향이 민감해져 방향이 크게 흔들렸다.
2. **도킹 지점에서 제자리 회전을 했다.** 팔이 든 팔레트는 차체 밖으로 많이 나와 있어, 차체가 들어갈 자리여도 팔레트가 컨베이어에 걸린다. 라이다는 그 높이를 자기 부피로 걸러내므로 미리 알 수도 없다. 걸린 뒤에도 30초 동안 계속 회전 명령을 냈다.

이번 조치는 다음과 같다.

| 조치 | 내용 |
|---|---|
| 후진 조향 | 멀리서는 목표점을 겨누고 가까워질수록 면과 직각 맞추기로 넘어가는 방식으로 바꾸고, 조향 상한을 0.20 rad/s 로 묶었다 |
| 도킹 지점 회전 금지 | 도착 시 방향이 3도 넘게 틀어져 있으면 그 자리에서 돌리지 않고 **면에서 0.6 m 물러나 다시 맞춘 뒤 재진입**한다(최대 2회) |
| 멈춤 감지 | 명령을 내는데 3초 동안 실제로 움직이지 않으면 즉시 멈추고 `STALLED_*` 로 실패한다. 구조물을 계속 밀지 않는다 |

내피 모의에서 재시도 경로를 강제로 태워 본 결과, 1차 도착 방향 오차 1.5도가 재진입 뒤 0.1도로 줄었다.
