# guidance2_24차 — 주행·도킹·Place 전 구간 (고피에서 지시, 내피에서 Nav2)

- 작성일: 2026-09-23, 브랜치 `feature/Inspection-Place-nav2`. 도킹이 후진 직후 멈춤으로 오판하던 원인을 고친 판이며 23차는 `guidance/past/` 로 옮김.
...
Nav2 주행은 성공했고, 정밀 도킹이 후진을 시작한 지 0.6초 만에 `STALLED_REVERSE` 로 끝났다. bag 기록은 다음과 같다.

| 시점 | 기록 |
|---|---|
| t=223.8 | `ALIGN_TO_GOAL` 시작. 제자리 회전으로 방향을 맞춤 |
| t=230.9 | `REVERSE` 시작. 목표점 뒤 1.14 m, 면까지 1.99 m, 방향 오차 0.0도 |
| t=231.5 | `FAILED STALLED_REVERSE`. 면까지 여전히 1.996 m |

이 구간 `/cmd_vel` 은 회전 중 `v=0.000 w=-0.123`, 후진 중 `v=-0.150 w=+0.015` 로 **정상이었다.** 즉 멈춘 것이 아니라 멈췄다고 잘못 판정한 것이다.

원인은 22차에서 넣은 멈춤 감지가 **위치 변화만** 보았다는 점이다. 그 앞 단계인 제자리 회전은 위치가 거의 그대로이므로, 회전 7초 동안 멈춤 타이머가 계속 흘렀고 후진 0.6초 만에 3초 기준을 넘겼다.

| 조치 | 내용 |
|---|---|
| 방향 변화도 이동으로 인정 | `stall_turn_rad` 0.02 를 추가해 제자리 회전을 "움직이는 중" 으로 본다 |
| 단계마다 기준점 재설정 | 단계가 바뀌면 멈춤 기준점을 새로 잡아, 앞 단계의 타이머가 다음 단계로 넘어가지 않게 했다 |

Isaac 을 며칠 못 쓰므로 내피 합성 로봇으로 네 가지를 미리 확인했다.
...
과민(정상 동작을 멈춤으로 봄)과 둔감(진짜 끼임을 못 봄) 양쪽을 모두 확인한 것이다.

### 앞뒤 용어에 대해 (읽기만)

도킹 구간에서 쓰는 `-v` 는 `base_link` 의 −x 방향이다. 리프트가 달린 쪽으로 정면 진입하면 자체 충돌이 나므로 의도적으로 택한 진입 방향이며, Nav2 는 접근 지점까지만 오고 거기서부터 도킹 노드가 맡는다. 장면에서 눈으로 본 앞뒤와 좌표계의 앞뒤를 초기에 반대로 잡았던 적이 있으니, 기록은 항상 좌표계 기준으로 읽는다.

### 지난 판에서 이미 고친 것 (읽기만)

| 판 | 증상 | 조치 |
|---|---|---|
| 22차 | 후진 1 m 중 방향 25도 틀어짐 → 컨베이어에 걸림 | 거리에 따라 조향을 섞고 상한 0.20 rad/s. 도킹 지점 제자리 회전 금지, 0.6 m 물러나 재진입(최대 2회) |
| 21차 | 도킹이 TurnTable 에 너무 가까움 | 도킹 거리 0.75 → 0.85 m (팔 밑동~대상 0.96 m) |
