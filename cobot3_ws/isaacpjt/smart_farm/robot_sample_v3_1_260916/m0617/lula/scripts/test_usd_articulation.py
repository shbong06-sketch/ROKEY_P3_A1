...
print("    ", "OK (2deg 미만)" if (err_deg < 2.0).all() else
      "FAIL -- 튐/추종 실패. 원인 후보: drive stiffness/damping, max force, joint axis 반전, "
      "질량/관성(자동계산 기준이면 collision mesh 품질), solver iteration, physics timestep. "
      "Lula 붙이기 전에 이것부터 원인 파악")
