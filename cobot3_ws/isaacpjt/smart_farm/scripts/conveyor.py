...
def install(stage, pallet_paths, vision_x=VISION_X,
            auto_resume=AUTO_RESUME_SECONDS, speed=DEFAULT_SPEED, carried_paths=()):
    ...
    carried_paths: [올인원 2026-09-25] pallet_paths 중 로봇이 포크로 들고 옮기는 팔레트.
                   운반 중에는 팔이 팔레트를 돌려야 하므로(CARRY_ROTATE) 요 고정을
                   벨트에 올라온 순간에 겁니다. 나머지는 지금처럼 바로 고정합니다.
    """
    prepare_world(stage)
    ...
    for path in pallet_paths:
        controller.watch(path, lock_yaw_now=path not in carried_paths)
...
class ConveyorController:
    ...
    def watch(self, pallet_path, lock_yaw_now=True):
        """이 팔레트를 감시 대상에 넣고, 컨베이어를 탈 수 있게 물성을 맞춥니다.

        팔레트 위 상추도 각각 리지드바디라 함께 처리합니다.
        lock_yaw_now=False 면 요 고정을 벨트에 올라온 순간(_update_zone)에 겁니다.
        """
        ...
        pallet = _Pallet(pallet_path, tray, [prim.GetPath().pathString for prim in bodies])
        pallet.lock_yaw_pending = not lock_yaw_now
        self._pallets.append(pallet)
        print(f"[컨베이어] {root.GetName()} 감시 시작: 리지드바디 {len(bodies)}개 · "
              f"{'요 고정' if lock_yaw_now else '요 고정은 벨트에 올라오면'}")
    ...
    def hold_stem(self, hold):
        """[올인원 2026-09-25] 줄기 벨트를 세우거나(True) 다시 돌립니다(False).

        벨트는 계속 돌기 때문에, 로봇이 줄기에 팔레트를 내려놓는 동안 포크가 아직 팔레트 안에 있어도
        벨트가 팔레트를 끌고 간다. 그러면 로봇 쪽 '포크를 빼는 중 팔레트가 따라옴' 검사에 걸리고
        포크 날이 팔레트를 긁는다. 실제 설비의 로봇-컨베이어 인터록처럼 놓는 동안만 세운다.
        """
        hold = bool(hold)
        if hold != self._stem_hold:
    ...
    def update(self, dt):
        ...
        driving = self._held() is None and not self._stem_hold
        for pallet in self._pallets:
    ...
    def _update_zone(self, pallet, dt, driving=True):
        ...
        if zone is not pallet.zone:
            ...
            if zone is Zone.VISION:
                ...
                if getattr(pallet, "lock_yaw_pending", False):
                    # [올인원 2026-09-25] 로봇이 내려놓은 팔레트: 이제부터 가이드 사이에서 방향 유지
                    PhysxSchema.PhysxRigidBodyAPI.Apply(pallet.tray).CreateLockedRotAxisAttr().Set(LOCKED_ROT_Z)
                    pallet.lock_yaw_pending = False
