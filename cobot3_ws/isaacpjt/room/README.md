...
| `lula/config/` | `robot_description.yaml`을 저장할 자리 (아직 비어있음 — XRDF Editor로 직접 생성 필요) |
...
| robot_description.yaml 생성 | ⏳ 대기(GUI 작업 필요) |
| yaml `cspace:` 조인트명 ↔ URDF 이름 검증 | ⏳ yaml 나오면 `check_yaml_joint_names.py`로 즉시 확인 |
