...
## Known Issues
- `config/m0617_robot_description.yaml` 없음 — 아래 "아직 안 된 것" 참고
...
## 아직 안 된 것
`robot_description.yaml`(collision sphere, c-space 등)은 이 URDF로부터 **Isaac Sim 안에서
Lula Robot Description(XRDF) Editor를 직접 열어 생성**해야 함 -- 자동 생성 로직이 메쉬 형상을
분석해서 구를 피팅하는 방식이라 GUI 없이 정확하게 만들어낼 수 없음. 위 2번 항목(`base` 링크)만
주의해서 만들면 예전에 막혔던 지점은 통과할 것으로 예상. 생성한 파일은 이 폴더와 같은 레벨의
`../config/m0617_robot_description.yaml`로 저장할 것 -- `ik_bridge.py`의 `ROBOT_DESCRIPTION_YAML`
기본값이 그 경로를 그대로 가리키고 있어서, 파일만 그 자리에 놓으면 코드 수정 없이 바로 연결됨.
