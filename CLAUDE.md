...
- 팀은 ROS 2 부트캠프 **입문자**임. `rclpy` 노드·토픽·파라미터·타이머·런치·평범한 파이썬 클래스 범위를 넘지 않음. 메타프로그래밍, 데코레이터(`@dataclass`/`@property` 제외), 불필요한 스레드/asyncio 금지.
- **문제를 수학·기하로 과하게 풀지 않음** (2026-09-25 재확인). 보정식·좌표 변환·추정 로직을 쌓기 전에 ROS 2 가 이미 주는 방법(파라미터, 토픽, 런치 인자, Nav2 기본 동작)으로 풀 수 있는지 먼저 봄. 식을 꼭 써야 하면 한 줄짜리 의미가 분명한 형태로 쓰고 무엇을 계산하는지 주석으로 남김. 코드는 짧고 읽히는 쪽을 택함.

---

## 2-1. 실측 기회의 보존 (2026-09-25)

Isaac Sim 실측은 사용자의 물리적 작업 시간이고 **영상 녹화 자료**이기도 함.

- **유의미한 트러블슈팅이 될 상황은 먼저 소진하지 말고 사용자가 직접 실측·녹화할 수 있게 남겨 둠.** 처음 보는 실패, 원인이 갈리는 증상, 절차 전체를 한 번 통과시키는 실행이 여기 해당함.
- 알아서 해결해도 되는 것: 자잘한 수치, 계산에 의한 오차, 모의로 확인 가능한 회귀, 빌드·설치 문제.
- **Isaac Sim 을 직접 실행해야 할 것 같으면 실행 전에 사용자에게 알리고 승인을 받음.**
